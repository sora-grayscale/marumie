...
export default function BalanceSheetChart({ assets, liabilities, className }: BalanceSheetChartProps) {
  ...
  const totalAssets = safeAssets.reduce((sum, item) => sum + item.value, 0);
  const totalLiabilities = safeLiabilities.reduce((sum, item) => sum + item.value, 0);
  const netWorth = totalAssets - totalLiabilities;

  if (!safeAssets.length && !safeLiabilities.length) {
    return (
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="h-[250px]">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={safeAssets} cx="50%" cy="50%" innerRadius={40} outerRadius={80} dataKey="value" label={({ name }) => name}>
                {safeAssets.map((entry) => (<Cell key={entry.name} fill={entry.color} />))}
              </Pie>
        ...
        {safeLiabilities.length > 0 && (
          <div className="h-[250px]">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={safeLiabilities} cx="50%" cy="50%" innerRadius={40} outerRadius={80} dataKey="value" label={({ name }) => name}>
                  {safeLiabilities.map((entry) => (<Cell key={entry.name} fill={entry.color} />))}
                </Pie>
                <Tooltip contentStyle={{ backgroundColor: "#1f2937", border: "1px solid #374151", borderRadius: "8px", color: "#f9fafb" }} formatter={(value: number) => [formatCurrency(value)]} />
              </PieChart>
            </ResponsiveContainer>
